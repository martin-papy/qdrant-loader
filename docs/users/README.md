# User Documentation

This section is for day-2 usage: configuration depth, source-specific guides, workflows, and troubleshooting.

If you are new, start with the [Quick Start Guide](../getting-started/quick-start.md).

## Main sections

- [Configuration](./configuration/README.md)
- [CLI reference](./cli-reference/commands.md)
- [Data source guides](./detailed-guides/data-sources/README.md)
- [MCP guide](./detailed-guides/mcp-server/README.md)
- [Workflows](./workflows/common-workflows.md)
- [Troubleshooting](./troubleshooting/README.md)

## Recommended reading order

1. [Configuration](./configuration/README.md)
2. [Data sources](./detailed-guides/data-sources/README.md)
3. [CLI reference](./cli-reference/commands.md)
4. [MCP guide](./detailed-guides/mcp-server/README.md)
5. [Troubleshooting](./troubleshooting/README.md)
