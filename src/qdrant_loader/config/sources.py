...
class SourcesConfig(BaseModel):
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "public_docs": {name: config.dict() for name, config in self.public_docs.items()},
            "git_repos": {name: config.dict() for name, config in self.git_repos.items()},
            "confluence": {name: config.dict() for name, config in self.confluence.items()},
            "jira": {name: config.dict() for name, config in self.jira.items()},
        }
