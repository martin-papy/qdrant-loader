...
class BatchSummary:
    ...
    def update_batch_stats(
        ...
        success_count: int,
        error_count: int,
        document_sizes: Optional[List[int]] = None,
        ...
        if source:
            self.source_counts[source] = self.source_counts.get(source, 0) + num_documents
            source_success = self.source_success_rates.get(source, 0.0) * self.source_counts[source]
            source_success += success_count
            self.source_success_rates[source] = source_success / self.source_counts[source]
    ...
    def get_summary(self) -> Dict[str, Union[Dict[str, Union[int, float]], Dict[str, Dict[str, float]]]]:
