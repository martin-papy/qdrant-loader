...
class StateManager:
    ...
    def get_document_states(
        self, source_type: str, source_name: str, since: Optional[datetime] = None
    ) -> List[DocumentState]:
    ...
    async def update_document_state(self, state: DocumentState) -> DocumentState:
        ...
        async with self.AsyncSession() as session:
            ...
            if existing:
                ...
                existing.updated_at = datetime.now(UTC)  # type: ignore
                state = existing
            else:
                # Create new record
        ...
        async with self.AsyncSession() as session:
            ...
            if not state:
                raise StateNotFoundError(
                    f"Document state not found for {source_type}/{source_name}/{document_id}"
                )
