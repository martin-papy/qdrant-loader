...
class BaseChunkingStrategy(ABC):
    ...
    def _should_apply_nlp(self, content: str, file_path: str = "", content_type: str = "") -> bool:
    ...
    def _create_chunk_document(
        ...
        if not should_apply_nlp:
            ...
            try:
                ...
                nlp_content = self._extract_nlp_worthy_content(chunk_content, element_type)
                    ...
                    metadata.update(
                        {
                            ...
                            "nlp_skipped": False,
                            "nlp_content_extracted": len(nlp_content) < len(chunk_content),
                            "nlp_content_ratio": (
                                len(nlp_content) / len(chunk_content) if chunk_content else 0
                            ),
                        }
                    )
                else:
