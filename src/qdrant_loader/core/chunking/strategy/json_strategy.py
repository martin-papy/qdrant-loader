...
class JSONChunkingStrategy(BaseChunkingStrategy):
    ...
    def _parse_json_structure(self, content: str) -> Optional[JSONElement]:
        ...
        if len(content) > MAX_JSON_SIZE_FOR_PARSING:
            self.logger.info(f"JSON too large for structured parsing ({len(content)} bytes)")
            return None
    ...
    def _create_json_element(
        self, name: str, value: Any, element_type: JSONElementType, path: str, level: int = 0
    ) -> JSONElement:
        ...
        if isinstance(data, dict):
            for i, (key, value) in enumerate(data.items()):
                ...
                if isinstance(value, (dict, list)):
                    ...
                    element_type = (
                        JSONElementType.OBJECT if isinstance(value, dict) else JSONElementType.ARRAY
                    )
            ...
            for i, item in enumerate(data):
                ...
                if isinstance(item, (dict, list)):
                    ...
                    element_type = (
                        JSONElementType.OBJECT if isinstance(item, dict) else JSONElementType.ARRAY
                    )
                    child_element = self._create_json_element(
                    ...
                    child_element = self._create_json_element(
                        f"item_{i}", item, JSONElementType.ARRAY_ITEM, child_path, level + 1
                    )
                    parent_element.add_child(child_element)
    ...
    def _create_grouped_element(self, elements: List[JSONElement]) -> JSONElement:
        ...
        parent_path = ".".join(base_path.split(".")[:-1]) if "." in base_path else "root"
    ...
    def _extract_json_metadata(self, element: JSONElement) -> Dict[str, Any]:
        ...
        if isinstance(element.value, dict):
            metadata["data_types"] = list(set(type(v).__name__ for v in element.value.values()))
            ...
            metadata["data_types"] = list(set(type(v).__name__ for v in element.value))
