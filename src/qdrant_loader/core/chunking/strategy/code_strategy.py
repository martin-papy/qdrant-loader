...
class CodeChunkingStrategy(BaseChunkingStrategy):
    ...
    def _extract_ast_elements(
        self, node, content: str, elements: List[CodeElement], language: str, level: int = 0
    ):
    ...
    def _parse_python_ast(self, content: str) -> List[CodeElement]:
        ...
        # Performance check: skip AST parsing for very large files
        if len(content) > MAX_FILE_SIZE_FOR_AST:
            self.logger.info(f"Python file too large for AST parsing ({len(content)} bytes)")
            return []
        ...
        def visit_node(node, level=0, parent_element=None):
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if isinstance(node, ast.Import):
                    import_names = [alias.name for alias in node.names[:10]]  # Limit imports
                else:
                    module = node.module or ""
                    import_names = [f"{module}.{alias.name}" for alias in node.names[:10]]
    ...
    def _extract_code_metadata(self, element: CodeElement, language: str) -> Dict[str, Any]:
        ...
        if element.element_type == CodeElementType.CLASS:
            metadata.update(
                {
                    "method_count": len(
                        [c for c in element.children if c.element_type == CodeElementType.METHOD]
                    ),
                    "property_count": len(
                        [c for c in element.children if c.element_type == CodeElementType.PROPERTY]
                    ),
                }
            )
    ...
    def _merge_small_elements(
        ...
        for element in elements:
            ...
            if (
                ...
                or element.element_type in [CodeElementType.CLASS, CodeElementType.FUNCTION]
                or (element.element_type == CodeElementType.METHOD and element_size > 100)
            ):
