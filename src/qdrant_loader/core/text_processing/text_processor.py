...
class TextProcessor:
    ...
    def process_text(self, text: str) -> dict:
        ...
        try:
            ...
            tokens = [token.text for token in doc][:MAX_POS_TAGS_TO_EXTRACT]  # Limit tokens
            ...
            return {"tokens": tokens, "entities": entities, "pos_tags": pos_tags, "chunks": chunks}
        except Exception as e:
            logger.warning(f"Text processing failed: {e}")
            # Return minimal results on error
            return {"tokens": [], "entities": [], "pos_tags": [], "chunks": [text] if text else []}
    ...
    def get_entities(self, text: str) -> List[tuple]:
        ...
        try:
            doc = self.nlp(text)
            return [(ent.text, ent.label_) for ent in doc.ents][:MAX_ENTITIES_TO_EXTRACT]
        except Exception as e:
