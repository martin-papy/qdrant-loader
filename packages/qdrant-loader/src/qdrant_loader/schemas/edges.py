...
class ContainsEdge(DocumentRelationshipEdge):
    ...
    container_section: Optional[str] = Field(
        None, description="Section of container that includes target"
    )
    position_index: Optional[int] = Field(None, description="Position within container")
...
class ReferencesEdge(DocumentRelationshipEdge):
    ...
    page_number: Optional[int] = Field(None, description="Page where reference occurs")
    line_number: Optional[int] = Field(None, description="Line where reference occurs")

    # Citation metadata
    citation_style: Optional[str] = Field(None, description="Citation style used")
...
class BelongsToEdge(EntityEdge):
    ...
    start_date: Optional[datetime] = Field(None, description="When membership started")
    end_date: Optional[datetime] = Field(None, description="When membership ended")

    # Role and permissions
    role_in_group: Optional[str] = Field(None, description="Role within the group")
    permissions: List[str] = Field(
        default_factory=list, description="Permissions granted"
    )
    access_level: Optional[str] = Field(None, description="Access level within group")
...
class RelatedToEdge(DocumentRelationshipEdge):
    ...
    # Discovery metadata
    discovered_through: Optional[str] = Field(
        None, description="How relationship was discovered"
    )
    discovery_algorithm: Optional[str] = Field(
