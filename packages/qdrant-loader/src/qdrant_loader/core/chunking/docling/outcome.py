...
heading-contextualised text that will actually be embedded, and the
...
class Chunk:
    ...
    # contextualize() output. Contextual embedding is NOT YET IMPLEMENTED, so this
    # mirrors text while the toggle is off; it is the carrier for when it lands.
    embed_text: str
    structure: ChunkStructure
    token_count: int | None = None  # tokens in embed_text — drives the budget check
