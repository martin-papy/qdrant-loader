...
class AsyncIngestionPipeline:
    ...
    async def process_documents(
        ...
        try:
            ...
            if filtered_config.git and not self._shutdown_event.is_set():
                ...
                logger.info(
                    f"Completed processing Git sources, got {len(git_docs)} documents"
                )
            ...
            logger.info(
                f"Completed processing all sources, total documents: {len(documents)}"
            )

            # Check for shutdown before change detection
            if self._shutdown_event.is_set():
                logger.info(
                    "Shutdown requested, skipping change detection and processing"
                )
                return documents
            ...
            if documents:
                logger.debug(
                    f"Starting change detection for {len(documents)} documents"
                )
                try:
                    ...
                    async with StateChangeDetector(
                        ...
                        logger.debug(
                            "StateChangeDetector initialized, detecting changes..."
                        )
                    ...
                    logger.error(
                        f"Error during change detection: {str(e)}", exc_info=True
                    )
                    raise
            ...
            if documents or deleted_documents:
                ...
                if documents and not self._shutdown_event.is_set():
                    ...
                    if not self._shutdown_event.is_set():
                        ...
                        successfully_processed_docs = [
                            ...
                            if doc.id in successfully_processed_documents
                        ]
                        ...
                        for doc in successfully_processed_docs:
                            try:
                                ...
                                logger.error(
                                    f"Failed to update document state for {doc.id}: {e}"
                                )
                ...
                if deleted_documents and not self._shutdown_event.is_set():
                    ...
                    for doc in deleted_documents:
                        if self._shutdown_event.is_set():
                            logger.info(
                                "Shutdown requested, stopping deleted document processing"
                            )
                            break
                        try:
                            # Delete points from Qdrant
                            await self.qdrant_manager.delete_points_by_document_id(
                                [doc.id]
                            )
            ...
            try:
                ...
                try:
                    ...
                    async with connector:
                        ...
                        logger.debug(
                            f"Documents list length after extend: {len(documents)}"
                        )
                        logger.debug(
                        ...
                        logger.debug(
                            f"Completed processing {source_type} source: {name}"
                        )
                    ...
                    self.monitor.end_operation(
                        f"source_{name}", success=False, error=str(e)
                    )
                    raise
    ...
    async def process_documents_pipeline(
        ...
        """
        logger.debug(
            f" Starting process_documents_pipeline with {len(documents)} documents"
        )
        ...
        async def chunker_worker(doc):
            ...
            try:
                ...
                with prometheus_metrics.CHUNKING_DURATION.time():
                    ...
                    if (
                        doc_size < 1_000
                    ):  # Very small files (< 1KB) - like our failing Java files
                        base_timeout = 10.0
                    ...
                    size_factor = min(
                        doc_size / 50000, 4.0
                    )  # Up to 4x for very large files
                    pressure_factor = min(
                        queue_pressure * 2, 2.0
                    )  # Up to 2x for queue pressure

                    # Final adaptive timeout
                    adaptive_timeout = base_timeout * (
                        1 + size_factor + pressure_factor
                    )
        ...
        async def chunker():
            ...
            try:
                ...
                if not self._shutdown_event.is_set():
                    ...
                    logger.debug("Chunker put all sentinels into chunk_queue")
                else:
                    logger.debug(
                        "Chunker exiting due to shutdown, putting emergency sentinels"
                    )
                    for _ in range(self.max_embed_workers):
                        try:
                ...
                logger.debug("Chunker cancelled")
                ...
                while not self._shutdown_event.is_set():
                    ...
                    for _ in range(batch_size):
                        try:
                            ...
                            if chunk is None:
                                logger.debug(
                                    f"Embedder_worker {worker_id} received sentinel"
                                )
                            ...
                            if self._shutdown_event.is_set():
                                logger.debug(
                                    f"Embedder_worker {worker_id} exiting due to shutdown"
                                )
                                return
                    ...
                    if batch:
                        try:
                            ...
                            logger.error(
                                f"Embedder_worker {worker_id} timed out processing batch"
                            )
                            for chunk in batch:
                                logger.error(
                                    f"Embedding timed out for chunk {chunk.id}"
                                )
                            ...
                            logger.error(
                                f"Embedder_worker {worker_id} error processing batch: {e}"
                            )
                            for chunk in batch:
                                logger.error(
                                    f"Embedding failed for chunk {chunk.id}: {e}"
                                )
                                # Mark parent document as failed
                                parent_doc = chunk.metadata.get("parent_document")
                                if parent_doc:
            ...
            try:
                while not self._shutdown_event.is_set():
                    try:
                                ...
                                try:
                                    with prometheus_metrics.UPSERT_DURATION.time():
                                        ...
                                        for chunk, _ in batch:
                                            parent_doc = chunk.metadata.get(
                                                "parent_document"
                                            )
                                            if parent_doc:
                                                successfully_processed_documents.add(
                                                    parent_doc.id
                                                )
                                except Exception as e:
                                    for chunk, _ in batch:
                                        logger.error(
                                            f"Upsert failed for chunk {chunk.id}: {e}"
                                        )
                                        # Mark parent document as failed
                                        parent_doc = chunk.metadata.get(
                                            "parent_document"
                        ...
                        if self._shutdown_event.is_set():
                            logger.debug(
                                f" upserter_worker {worker_id} exiting due to shutdown"
                            )
                            break
                        continue
            except asyncio.CancelledError:
        ...
        logger.debug(" Creating chunker task")
        ...
        embedder_tasks = [
            asyncio.create_task(embedder_worker(i))
            for i in range(self.max_embed_workers)
        ]
        ...
        upserter_tasks = [
            asyncio.create_task(upserter_worker(i))
            for i in range(self.max_upsert_workers)
        ]
        ...
        async def emergency_shutdown():
            ...
            try:
                ...
                logger.warning(
                    " Some tasks did not complete within emergency shutdown timeout"
                )

        try:
            # Wait for chunker to complete
            logger.debug(" Awaiting chunker_task")
            await chunker_task

            # Wait for embedders to complete
            logger.debug(" Awaiting embedder_tasks")
            ...
            if not self._shutdown_event.is_set():
                ...
                logger.debug(" embedder put all sentinels into embed_queue")

            # Wait for upserters to complete
            logger.debug(" Awaiting upserter_tasks")
            await asyncio.gather(*upserter_tasks)
            logger.debug(" upserter_tasks completed")

        except (asyncio.CancelledError, KeyboardInterrupt):
            logger.warning(
                " Received cancellation or SIGINT, initiating emergency shutdown..."
            )
        ...
        logger.info(
            f"Pipeline completed: {success_count} success, {error_count} errors"
        )
        logger.info(
