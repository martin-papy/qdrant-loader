...
class HybridSearchService:
    ...
    async def _expand_query(self, query: str) -> str:
        ...
        for key, expansions in self.query_expansions.items():
            if key.lower() in lower_query:
                ...
                self.logger.debug(
                    "Expanded query",
                    extra={"original_query": query, "expanded_query": expanded_query},
                )
                break
    ...
    async def search(
        ...
        try:
            ...
            vector_results = await self._vector_search(
                expanded_query, limit * 3, filter_conditions
            )

            # Get keyword search results with original query for precision
            keyword_results = await self._keyword_search(
                query, limit * 3, filter_conditions
            )
            ...
            combined_results = self._combine_results(
                vector_results, keyword_results, query_context, limit
            )
    ...
    def _analyze_query(self, query: str) -> dict[str, Any]:
        ...
        lower_query = query.lower()
        context["keywords"] = [
            word.lower() for word in re.findall(r"\b\w{3,}\b", lower_query)
        ]
    ...
    async def _vector_search(
        self, query: str, limit: int, filter_conditions: dict[str, Any] | None = None
    ) -> list[dict[str, Any]]:
        ...
        search_params = models.SearchParams(
            hnsw_ef=128, exact=False  # Increased for better recall
        )
    ...
    def _calculate_metadata_score(
        self, metadata: dict[str, Any], query_context: dict[str, Any]
    ) -> float:
        ...
        if query_context.get("is_broad", False):
            ...
            if any(
                ...
                for term in ["introduction", "overview", "about", "purpose", "summary"]
            ):
                score += 0.25
        ...
        if query_context.get("probable_intent") == "requirements":
            ...
            if any(
                term in section_title
                for term in ["requirement", "feature", "goal", "objective"]
            ):
                score += 0.3
            elif any(
                term in section_path
                for term in ["requirement", "feature", "goal", "objective"]
            ):
                score += 0.2
            elif any(
    ...
    def _combine_results(
        ...
        for _doc_id, doc_info in combined_dict.items():
            metadata_score = self._calculate_metadata_score(
                doc_info["metadata"], query_context
            )
