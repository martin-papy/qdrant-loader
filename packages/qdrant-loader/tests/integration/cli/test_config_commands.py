...
class TestConfigShow:
    ...
    def test_config_show_without_config(
        ...
        result = cli_runner.invoke(
            cli_app, ["config", "show", "--workspace", str(temp_workspace)]
        )
    ...
    def test_config_show_legacy_mode(
        self, cli_runner: CliRunner, cli_app, temp_workspace: Path
    ):
...
"""
        ...
        result = cli_runner.invoke(
            cli_app, ["config", "show", "--config", str(config_file)]
        )
...
class TestConfigValidate:
    ...
    def test_config_validate_without_config(
        self, cli_runner: CliRunner, cli_app, temp_workspace: Path
    ):
...
class TestConfigGenerate:
    ...
    def test_config_generate_basic(
        self, cli_runner: CliRunner, cli_app, temp_workspace: Path
    ):
    ...
    def test_config_generate_with_template(
        self, cli_runner: CliRunner, cli_app, temp_workspace: Path
    ):
    ...
    def test_config_generate_existing_config(
        self, cli_runner: CliRunner, cli_app, workspace_with_config: Path
    ):
...
"""
    ...
    def test_config_with_env_option(
        self, cli_runner: CliRunner, cli_app, temp_workspace: Path
    ):
        ...
        result = cli_runner.invoke(cli_app, ["config", "show", "--env", str(env_file)])
...
"""
    ...
    def test_mixed_config_detection(
        self, cli_runner: CliRunner, cli_app, temp_workspace: Path
    ):
...
class TestConfigWorkspaceOperations:
    ...
    def test_workspace_config_structure(
        self, cli_runner: CliRunner, cli_app, temp_workspace: Path
    ):
...
class TestConfigErrorScenarios:
    ...
    def test_config_permission_error(self, cli_runner: CliRunner, cli_app):
        ...
        restricted_path = (
            "/root/restricted"
            if os.name != "nt"
            else "C:\\Windows\\System32\\restricted"
        )

        result = cli_runner.invoke(cli_app, ["config", "--workspace", restricted_path])

        # Should handle permission errors gracefully
        assert result.exit_code != 0
        # Output might be empty due to internal errors, so don't assert on it

    def test_config_corrupted_file(
        self, cli_runner: CliRunner, cli_app, temp_workspace: Path
    ):
