...
class TestCreateDatabaseDirectoryAdvanced:
    ...
    def test_create_database_directory_permission_error(self, mock_confirm):
        ...
        test_path = Path(
            "/root/forbidden_directory"
        )  # Path that would need root access
        mock_confirm.return_value = True
...
class TestUtilityPatterns:
    ...
    def test_constants_and_options(self):
        ...
        assert callable(WORKSPACE_OPTION)
        assert callable(CONFIG_OPTION)
        assert callable(ENV_OPTION)
        assert callable(LOG_LEVEL_OPTION)
        assert callable(DOMAINS_OPTION)
        assert callable(PRESET_OPTION)
        assert callable(USE_CASE_OPTION)
        assert callable(PERFORMANCE_OPTION)
