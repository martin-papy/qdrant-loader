...
class TestPublicDocsConnector:
        ...
        assert (
            len(
                [
                    link
                    for link in links
                    if link.startswith(str(publicdocs_config.base_url))
                ]
            )
            == 2
        )
    ...
    async def test_get_documents(
        ...
        assert (
            len(documents) == 2
        )  # We should get both the base page and the linked page
    ...
    async def test_get_documents_error_handling(
        ...
        with patch("aiohttp.ClientSession", return_value=session):
            async with connector:
                ...
                with patch.object(
                    connector, "_get_all_pages", side_effect=mock_get_all_pages
                ):
        ...
        with patch("aiohttp.ClientSession", return_value=session):
            async with connector:
                ...
                with patch.object(
                    connector,
                    "_extract_content",
                    side_effect=Exception("Content processing error"),
                ):
