...
class TestValidationIntegrationManager:
    ...
    @pytest.mark.asyncio
    async def test_trigger_post_ingestion_validation_no_integrator(self, validation_manager):
    ...
    @pytest.mark.asyncio
    async def test_trigger_post_ingestion_validation_success(self, validation_manager_with_integrator):
    ...
    @pytest.mark.asyncio
    async def test_trigger_post_sync_validation_failed_operation(self, validation_manager_with_integrator):
    ...
    @pytest.mark.asyncio
    async def test_trigger_post_sync_validation_should_not_validate(self, validation_manager_with_integrator):
        ...
        with patch.object(
            ...
            result = await validation_manager_with_integrator.trigger_post_sync_validation(operation)

        assert result is False

    @pytest.mark.asyncio
    async def test_trigger_post_sync_validation_success(self, validation_manager_with_integrator):
    ...
    @pytest.mark.asyncio
    async def test_trigger_change_event_validation_no_integrator(self, validation_manager):
    ...
    @pytest.mark.asyncio
    async def test_trigger_change_event_validation_should_not_validate(self, validation_manager_with_integrator):
        ...
        with patch.object(
            ...
            result = await validation_manager_with_integrator.trigger_change_event_validation(event)

        assert result is False

    @pytest.mark.asyncio
    async def test_trigger_change_event_validation_success(self, validation_manager_with_integrator):
    ...
    @pytest.mark.asyncio
    async def test_trigger_validation_with_delay_recent_validation(self, validation_manager_with_integrator):
        """Test triggering validation when recent validation exists."""
        validation_key = "test_validation"
        # Set a recent validation (within the delay period)
        validation_manager_with_integrator._validation_history[validation_key] = datetime.now(UTC)
        
        # Mock asyncio.create_task to avoid actual async task creation
        with patch('asyncio.create_task') as mock_create_task, \
             patch('asyncio.sleep') as mock_sleep, \
             patch.object(validation_manager_with_integrator, '_execute_validation_with_retry') as mock_retry:
    ...
    @pytest.mark.asyncio
    async def test_trigger_validation_with_delay_success(self, validation_manager_with_integrator):
    ...
    @pytest.mark.asyncio
    async def test_execute_validation_with_retry_success(self, validation_manager_with_integrator):
        """Test successful validation execution with retry."""
        validation_key = "test_validation"
        context = {"test": "data"}

        # Mock the actual validation method to avoid hanging
        with patch.object(validation_manager_with_integrator, '_execute_validation') as mock_execute:
    ...
    @pytest.mark.asyncio
    async def test_execute_validation_with_retry_failure_and_retry(self, validation_manager_with_integrator):
    ...
    @pytest.mark.asyncio
    async def test_execute_validation_with_retry_max_failures(self, validation_manager_with_integrator):
    ...
    @pytest.mark.asyncio
    async def test_execute_validation_success_no_issues(self, validation_manager_with_integrator):
        ...
        validation_manager_with_integrator.validation_integrator.trigger_validation.return_value = validation_result
        ...
        assert validation_key not in validation_manager_with_integrator._active_validations
    ...
    async def test_trigger_auto_repair_success(self, validation_manager_with_integrator):
        ...
        validation_manager_with_integrator.validation_integrator.run_auto_repair.return_value = [repair_result]
    ...
    @pytest.mark.asyncio
    async def test_trigger_auto_repair_failure(self, validation_manager_with_integrator):
        """Test auto-repair execution failure."""
        validation_result = MagicMock()
        validation_result.has_errors = True

        validation_manager_with_integrator.validation_integrator.run_auto_repair.side_effect = Exception("Repair failed")
    ...
    def test_should_validate_operation_delete(self, validation_manager):
        ...
        result = validation_manager._should_validate_operation(operation)
        assert result is True  # DELETE_DOCUMENT operations should be validated  # validate_on_delete is False
