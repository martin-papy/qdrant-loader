...
class TestBidirectionalSyncEngineCoveragePriority:
    ...
    def mock_managers(self):
        ...
        neo4j_manager.execute_write_query = MagicMock(
            return_value=[{"node_id": 123, "node_uuid": "test-uuid"}]
        )
    ...
    async def test_batch_processing_loop_basic(self, sync_engine_batch):
        ...
        with (
            patch.object(
                sync_engine_batch, "_process_pending_operations"
            ) as mock_process,
    ...
    async def test_process_batch_grouping_by_direction(self, sync_engine_batch):
        ...
        with (
            patch.object(
                sync_engine_batch, "_process_qdrant_to_neo4j_batch"
            ) as mock_q2n,
            patch.object(
                sync_engine_batch, "_process_neo4j_to_qdrant_batch"
            ) as mock_n2q,
        ):
    ...
    async def test_process_batch_statistics_update(self, sync_engine_batch):
        ...
        with (
            patch.object(sync_engine_batch, "_process_qdrant_to_neo4j_batch"),
    ...
    @pytest.mark.asyncio
    async def test_execute_sync_operation_qdrant_to_neo4j(
        self, sync_engine_immediate, sample_mapping
    ):
        """Test executing QDrant to Neo4j sync operation."""
        operation = SyncOperation(
            direction=SyncDirection.QDRANT_TO_NEO4J, mapping=sample_mapping
        )

        with (
            patch.object(sync_engine_immediate, "_resolve_mapping") as mock_resolve,
    ...
    @pytest.mark.asyncio
    async def test_execute_sync_operation_neo4j_to_qdrant(
        self, sync_engine_immediate, sample_mapping
    ):
        """Test executing Neo4j to QDrant sync operation."""
        operation = SyncOperation(
            direction=SyncDirection.NEO4J_TO_QDRANT, mapping=sample_mapping
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_sync_operation_unsupported_direction(
        self, sync_engine_immediate
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_mapping_qdrant_existing(
        self, sync_engine_immediate, sample_mapping, sample_change_event
    ):
        """Test resolving mapping for QDrant entity with existing mapping."""
        operation = SyncOperation(event=sample_change_event)

        sync_engine_immediate.id_mapping_manager.get_mapping_by_qdrant_id.return_value = (
            sample_mapping
        )
        ...
        sync_engine_immediate.id_mapping_manager.get_mapping_by_qdrant_id.assert_called_once_with(
            "test_id"
        )

    @pytest.mark.asyncio
    async def test_resolve_mapping_neo4j_existing(
        self, sync_engine_immediate, sample_mapping
    ):
        ...
        sync_engine_immediate.id_mapping_manager.get_mapping_by_neo4j_id.return_value = (
            sample_mapping
        )
        ...
        assert operation.mapping == sample_mapping
        sync_engine_immediate.id_mapping_manager.get_mapping_by_neo4j_id.assert_called_once_with(
            "neo4j_id"
        )
    ...
    async def test_resolve_mapping_by_uuid(self, sync_engine_immediate, sample_mapping):
        ...
        sync_engine_immediate.id_mapping_manager.get_mapping_by_neo4j_uuid.return_value = (
            sample_mapping
        )
        ...
        assert operation.mapping == sample_mapping
        sync_engine_immediate.id_mapping_manager.get_mapping_by_neo4j_uuid.assert_called_once_with(
            "test-uuid"
        )

    @pytest.mark.asyncio
    async def test_resolve_mapping_create_new_for_create_operation(
        self, sync_engine_immediate, sample_mapping
    ):
        ...
        with patch.object(
            sync_engine_immediate,
            "_create_mapping_for_event",
    ...
    @pytest.mark.asyncio
    async def test_create_mapping_for_event_qdrant(
        self, sync_engine_immediate, sample_change_event
    ):
        ...
        sync_engine_immediate.id_mapping_manager.create_mapping.return_value = (
            new_mapping
        )

        result = await sync_engine_immediate._create_mapping_for_event(
            sample_change_event
        )

        assert result == new_mapping
        sync_engine_immediate.id_mapping_manager.create_mapping.assert_called_once()
        call_kwargs = sync_engine_immediate.id_mapping_manager.create_mapping.call_args[
            1
        ]
    ...
    async def test_sync_qdrant_to_neo4j_no_mapping(self, sync_engine_immediate):
        ...
        operation = SyncOperation(
            event=ChangeEvent(change_type=ChangeType.CREATE), mapping=None
        )
    ...
    @pytest.mark.asyncio
    async def test_sync_qdrant_to_neo4j_create_success(
        self, sync_engine_immediate, sample_mapping, sample_change_event
    ):
        ...
        with patch.object(
            sync_engine_immediate, "_create_neo4j_node_from_qdrant"
        ) as mock_create:
            await sync_engine_immediate._sync_qdrant_to_neo4j(operation)
    ...
    @pytest.mark.asyncio
    async def test_sync_qdrant_to_neo4j_update_success(
        self, sync_engine_immediate, sample_mapping
    ):
        ...
        with patch.object(
            sync_engine_immediate, "_update_neo4j_node_from_qdrant"
        ) as mock_update:
            await sync_engine_immediate._sync_qdrant_to_neo4j(operation)
    ...
    @pytest.mark.asyncio
    async def test_sync_qdrant_to_neo4j_delete_success(
        self, sync_engine_immediate, sample_mapping
    ):
        ...
        with patch.object(
            sync_engine_immediate, "_delete_neo4j_node_from_qdrant"
        ) as mock_delete:
            await sync_engine_immediate._sync_qdrant_to_neo4j(operation)
    ...
    @pytest.mark.asyncio
    async def test_sync_qdrant_to_neo4j_unsupported_change_type(
        self, sync_engine_immediate, sample_mapping
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_qdrant_to_neo4j_exception_handling(
        self, sync_engine_immediate, sample_mapping, sample_change_event
    ):
        ...
        with patch.object(
            sync_engine_immediate,
            "_create_neo4j_node_from_qdrant",
    ...
    @pytest.mark.asyncio
    async def test_sync_neo4j_to_qdrant_create_success(
        self, sync_engine_immediate, sample_mapping
    ):
        ...
        with patch.object(
            sync_engine_immediate, "_create_qdrant_point_from_neo4j"
        ) as mock_create:
            await sync_engine_immediate._sync_neo4j_to_qdrant(operation)
    ...
    @pytest.mark.asyncio
    async def test_create_neo4j_node_from_qdrant_missing_data(
        self, sync_engine_immediate, sample_mapping
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_neo4j_node_from_qdrant_success(
        self, sync_engine_immediate, sample_mapping, sample_change_event
    ):
        ...
        with patch.object(
            sync_engine_immediate,
            "_extract_neo4j_properties_from_qdrant_data",
    ...
    @pytest.mark.asyncio
    async def test_update_neo4j_node_from_qdrant_missing_node_id(
        self, sync_engine_immediate, sample_change_event
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_neo4j_node_from_qdrant_success(
        self, sync_engine_immediate, sample_mapping, sample_change_event
    ):
    ...
    @pytest.mark.asyncio
    async def test_delete_neo4j_node_from_qdrant_missing_node_id(
        self, sync_engine_immediate
    ):
    ...
    @pytest.mark.asyncio
    async def test_delete_neo4j_node_from_qdrant_success(
        self, sync_engine_immediate, sample_mapping
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_qdrant_point_from_neo4j_missing_data(
        self, sync_engine_immediate, sample_mapping
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_qdrant_point_from_neo4j_missing_vector(
        self, sync_engine_immediate, sample_mapping
    ):
        ...
        with patch.object(
            sync_engine_immediate,
            "_extract_qdrant_data_from_neo4j_properties",
            return_value={},
        ):
    ...
    @pytest.mark.asyncio
    async def test_create_qdrant_point_from_neo4j_success(
        self, sync_engine_immediate, sample_mapping
    ):
    ...
    def test_extract_neo4j_properties_from_qdrant_data(self, sync_engine_immediate):
        ...
        result = sync_engine_immediate._extract_neo4j_properties_from_qdrant_data(
            qdrant_data
        )
    ...
    def test_extract_qdrant_data_from_neo4j_properties(self, sync_engine_immediate):
        ...
        result = sync_engine_immediate._extract_qdrant_data_from_neo4j_properties(
            neo4j_data
        )
    ...
    async def test_start_with_batch_strategy_creates_task(self, sync_engine_batch):
        ...
        with patch.object(
            sync_engine_batch, "_batch_processing_loop", new_callable=AsyncMock
        ) as mock_loop:
    ...
    @pytest.mark.asyncio
    async def test_process_operation_immediate_updates_statistics(
        self, sync_engine_immediate
    ):
        ...
        with patch.object(
            sync_engine_immediate, "_execute_sync_operation"
        ) as mock_execute:
    ...
    @pytest.mark.asyncio
    async def test_process_operation_immediate_handles_exceptions(
        self, sync_engine_immediate
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_qdrant_to_neo4j_batch_handles_exceptions(
        self, sync_engine_batch
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_neo4j_to_qdrant_batch_handles_exceptions(
        self, sync_engine_batch
    ):
