...
class TestConflictResolutionStrategies:
    ...
    @pytest.mark.asyncio
    async def test_temporal_invalidation_conflict_resolution(self, temporal_manager, sample_entity):
        ...
        temporal_manager.default_strategy = ConflictResolutionStrategy.TEMPORAL_INVALIDATION
    ...
    @pytest.mark.asyncio
    async def test_latest_wins_conflict_resolution(self, temporal_manager, sample_entity):
    ...
    @pytest.mark.asyncio
    async def test_highest_confidence_conflict_resolution(self, temporal_manager, sample_entity):
        ...
        low_confidence_entity = ExtractedEntity(
            ...
            temporal_info=TemporalInfo(valid_from=datetime.now(UTC) - timedelta(hours=1))
        )
        ...
        high_confidence_entity = ExtractedEntity(
            ...
            temporal_info=TemporalInfo(valid_from=datetime.now(UTC) - timedelta(minutes=30))
        )
        
        # Set strategy to highest confidence
        temporal_manager.default_strategy = ConflictResolutionStrategy.HIGHEST_CONFIDENCE
        
        # Add conflicting entity
        entity2_uuid, conflicts = await temporal_manager.add_entity(high_confidence_entity)
    ...
    @pytest.mark.asyncio
    async def test_merge_attributes_conflict_resolution(self, temporal_manager, sample_entity):
    ...
    @pytest.mark.asyncio
    async def test_manual_review_conflict_resolution(self, temporal_manager, sample_entity):
    ...
    @pytest.mark.asyncio
    async def test_relationship_conflict_detection(self, temporal_manager, sample_relationship):
        ...
        conflicting_relationship = ExtractedRelationship(
            ...
            temporal_info=TemporalInfo(
                valid_from=datetime.now(UTC) - timedelta(minutes=30),  # Overlapping time
                version=1
            )
        )
        
        # Add conflicting relationship
        rel2_uuid, conflicts = await temporal_manager.add_relationship(conflicting_relationship)
...
class TestAdvancedQuerying:
    ...
    async def test_query_entities_with_sorting_and_limiting(self, temporal_manager):
        ...
        for i in range(5):
            entity = ExtractedEntity(
                ...
                context=f"Entity {i}",
                temporal_info=TemporalInfo(
                    valid_from=datetime.now(UTC) - timedelta(hours=i),
                    version=1
                )
            )
    ...
    async def test_query_relationships_with_filtering(self, temporal_manager):
        ...
        for i in range(3):
            relationship = ExtractedRelationship(
                ...
                relationship_type=RelationshipType.USES if i % 2 == 0 else RelationshipType.CONTAINS,
    ...
    def test_get_conflicts_filtering(self, temporal_manager):
        ...
        temporal_manager._conflicts[unresolved_conflict.conflict_id] = unresolved_conflict
...
class TestRollbackFunctionality:
    ...
    async def test_rollback_entity_to_version(self, temporal_manager, sample_entity):
        ...
        no_rollback = await temporal_manager.rollback_entity_to_version(entity1_uuid, 99)
    ...
    @pytest.mark.asyncio
    async def test_rollback_relationship_to_version(self, temporal_manager, sample_relationship):
        ...
        # Rollback to version 1
        rolled_back = await temporal_manager.rollback_relationship_to_version(rel1_uuid, 1)
        ...
        # Test rollback to non-existent version
        no_rollback = await temporal_manager.rollback_relationship_to_version(rel1_uuid, 99)
        assert no_rollback is None
    ...
    def test_compare_relationship_versions(self, temporal_manager):
        ...
        # Compare versions
        comparison = temporal_manager.compare_relationship_versions("compare-rel-uuid", 1, 2)
        ...
        no_comparison = temporal_manager.compare_relationship_versions("non-existent", 1, 2)
        assert no_comparison is None
...
class TestBulkOperations:
    ...
    async def test_bulk_rollback_entities(self, temporal_manager):
        ...
        rollback_ops = [
            ...
            {"entity_uuid": "non-existent", "target_version": 1},  # Should fail gracefully
        ]
    ...
    def test_prune_versions_helper(self, temporal_manager):
        ...
        for i in range(5):
            entity = ExtractedEntity(
                ...
                temporal_info=TemporalInfo(
                    version=i+1,
                    transaction_time=base_time - timedelta(days=i*5)
                )
            )
    ...
    async def test_query_entities_at_time_in_timezone(self, temporal_manager):
        ...
        try:
            results = await temporal_manager.query_entities_at_time_in_timezone(
                query_time_str="2024-01-15 12:00:00",
...
class TestAdvancedTemporalOperations:
    ...
    async def test_traverse_temporal_graph(self, temporal_manager):
        ...
        assert "paths" in traversal or "nodes" in traversal or "relationships" in traversal
