...
class TestEntityExtractorCoveragePriority:
    ...
    @pytest.mark.asyncio
    async def test_background_workers_initialization(self, mock_graphiti_manager, background_config):
    ...
    @pytest.mark.asyncio
    async def test_background_worker_processing(self, background_enabled_extractor, sample_extraction_tasks):
    ...
    @pytest.mark.asyncio
    async def test_process_extraction_task_timeout(self, background_enabled_extractor, sample_extraction_tasks):
    ...
    def test_parse_llm_response_complex_json(self, background_enabled_extractor):
        ...
        complex_response = json.dumps({
            ...
            "metadata": {"extraction_method": "advanced", "timestamp": "2023-01-01"}
        })
        
        entities = extractor._parse_llm_response_to_entities(complex_response, "source text")
    ...
    def test_parse_llm_response_confidence_filtering(self, background_enabled_extractor):
        ...
        entities = extractor._parse_llm_response_to_entities(low_confidence_response, "source text")
    ...
    @pytest.mark.asyncio
    async def test_perform_custom_prompt_extraction_comprehensive(self, background_enabled_extractor):
        ...
        mock_prompt_manager.get_extraction_hints_for_domain.return_value = ["hint1", "hint2"]
        ...
        extractor.graphiti_manager.llm_client.generate_response.return_value = llm_response
    ...
    @pytest.mark.asyncio
    async def test_perform_custom_prompt_extraction_no_llm_client(self, background_enabled_extractor):
    ...
    async def test_extract_relationships_complex_response(self, background_enabled_extractor):
        ...
        extractor.graphiti_manager.llm_client.generate_response.return_value = complex_relationship_response
        
        relationships = await extractor.extract_relationships(entities, "John Doe works at TechCorp on AI Research")
    ...
    @pytest.mark.asyncio
    async def test_extract_entities_async_queue(self, mock_graphiti_manager, background_config):
        ...
        try:
            ...
            with patch.object(extractor, 'extract_entities') as mock_extract:
                mock_extract.return_value = ExtractionResult(
                    entities=[ExtractedEntity(name="Queue Entity", entity_type=EntityType.CONCEPT)]
                )
    ...
    @pytest.mark.asyncio 
    async def test_extract_entities_streaming(self, mock_graphiti_manager, background_config):
        ...
        try:
            ...
            with patch.object(extractor, 'extract_entities_batch') as mock_batch:
                mock_batch.return_value = [
                    ExtractionResult(
                        entities=[ExtractedEntity(name=f"Stream Entity {i}", entity_type=EntityType.CONCEPT)]
                    ) for i in range(3)
                ]
    ...
    async def test_integration_test_comprehensive(self, background_enabled_extractor):
        ...
        with patch.object(extractor, 'extract_entities') as mock_extract:
            mock_extract.return_value = ExtractionResult(
                entities=[ExtractedEntity(name="Integration Test Entity", entity_type=EntityType.CONCEPT)],
                processing_time=0.1
            )
    ...
    @pytest.mark.asyncio
    async def test_extract_entities_batch_with_exceptions(self, background_enabled_extractor):
        ...
        async def mock_extract_with_error(text, *args, **kwargs):
            ...
            return ExtractionResult(entities=[ExtractedEntity(name="Success", entity_type=EntityType.CONCEPT)])
        
        with patch.object(extractor, 'extract_entities', side_effect=mock_extract_with_error):
    ...
    async def test_full_workflow_integration(self, background_enabled_extractor):
        ...
        with patch.object(extractor.graphiti_manager, 'add_episode') as mock_episode:
            ...
            with patch.object(extractor.graphiti_manager, 'get_entities_from_episode') as mock_entities:
                mock_node = Mock()
