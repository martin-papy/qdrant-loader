...
class TestGraphitiManager:
    ...
    async def test_initialize_already_initialized(self, graphiti_manager):
        ...
        assert graphiti_manager._graphiti is not None
    ...
    @pytest.mark.asyncio
    async def test_get_nodes_with_failure(self, graphiti_manager, mock_graphiti_client):
        """Test get_nodes with search failure."""
        graphiti_manager._graphiti = mock_graphiti_client
        graphiti_manager._llm_client = Mock()
        graphiti_manager._embedder = Mock()
        ...
        with pytest.raises(Exception, match="Search failed"):
            await graphiti_manager.get_nodes(["uuid-1"])
