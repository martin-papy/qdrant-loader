...
def test_get_package_version(temp_pyproject):
    ...
    with (
        ...
        mock_log.debug.assert_any_call(
            "Reading current version from packages/qdrant-loader/pyproject.toml"
        )
        mock_log.debug.assert_any_call("Current version for qdrant-loader: 0.1.0")
...
def test_run_command():
    ...
    with (
        ...
        assert stdout == "main"
        mock_log.debug.assert_called_with(
            "Executing command: git branch --show-current"
        )
...
def test_check_current_branch_other():
    ...
    with (
        ...
        with pytest.raises(SystemExit):
            check_current_branch()
        mock_log.error.assert_called_with(
            "Not on main branch. Please switch to main branch first."
        )
...
def test_create_github_release():
    ...
    with (
        ...
        # Test the function
        create_github_release("qdrant-loader", "0.1.0", "test-token")
...
def test_check_main_up_to_date_up_to_date():
    ...
    with (
        ...
        mock_run.side_effect = [
            ("", ""),
            ("0", ""),
        ]  # For git fetch  # For git rev-list
    ...
    for remote_url, expected_repo in test_cases:
        with (
            ...
            mock_log.info.assert_any_call(
                "GitHub workflows check completed successfully"
            )
...
def test_check_github_workflows_running():
    ...
    with (
        ...
        mock_log.error.assert_any_call(
            "- Test and Coverage is running: http://example.com"
        )
...
def test_dry_run_mode():
    ...
    with patch("release.logging.getLogger") as mock_logger:
        ...
        # Test update_package_version
        with patch("release.open") as mock_open:
            update_package_version("qdrant-loader", "0.2.0", dry_run=True)
            mock_open.assert_not_called()
            mock_log.info.assert_called_with(
                "[DRY RUN] Would update version in packages/qdrant-loader/pyproject.toml to 0.2.0"
            )
