...
def analyze_nested_metadata(client: QdrantClient, collection_name: str, limit: int = 3):
    ...
    try:
        ...
        print("Analyzing nested metadata from {len(points)} documents...\n")
        ...
        for i, point in enumerate(points):
            ...
            print("=== Document {i+1} ===")
            print("Document ID: {point.id}")
            print("Confluence Page ID: {nested_metadata.get('id', 'N/A')}")
            print("Title: {nested_metadata.get('title', 'N/A')}")
            print("Space: {nested_metadata.get('space', 'N/A')}")
            print("Type: {nested_metadata.get('type', 'N/A')}")
            print("Author: {nested_metadata.get('author', 'N/A')}")
            print("Version: {nested_metadata.get('version', 'N/A')}")
            print("Created: {nested_metadata.get('created_at', 'N/A')}")
            print("Updated: {nested_metadata.get('updated_at', 'N/A')}")
            print("Labels: {nested_metadata.get('labels', [])}")
            print("Comments: {len(nested_metadata.get('comments', []))} comments")
            ...
            for key in sorted(nested_metadata.keys()):
                ...
                if isinstance(value, list | dict):
                    print("  {key}: {type(value).__name__} (length: {len(value)})")
                else:
                    print("  {key}: {value}")
        ...
        print("Unique nested metadata fields found: {len(all_nested_keys)}")
        print("All nested fields: {sorted(all_nested_keys)}")
        ...
        for field in expected_confluence_fields:
            if field in all_nested_keys:
                ...
                status = (
                    ...
                    else "⚠️ ({analysis['null_count']} null/empty)"
                )
                print("{status} {field}")
            else:
                print("❌ {field} - MISSING")
