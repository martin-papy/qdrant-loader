...
async def test_keyword_search(hybrid_search, mock_qdrant_client):
    ...
    mock_points = [
        ...
        models.Record(
            id="2", payload={"content": "Another document about machine learning"}
        ),
    ]
...
async def test_hybrid_search(hybrid_search):
    ...
    keyword_results = [
        ...
        {"id": "3", "score": 0.7, "content": "Test content 3", "metadata": {}},
    ]
...
async def test_search_with_filters(hybrid_search):
    ...
    results = await hybrid_search.search(
        "test query", limit=1, filter_conditions=filter_conditions
    )
    ...
    hybrid_search._vector_search.assert_called_once_with(
        "test query", 2, filter_conditions
    )
    hybrid_search._keyword_search.assert_called_once_with(
        "test query", 2, filter_conditions
    )
