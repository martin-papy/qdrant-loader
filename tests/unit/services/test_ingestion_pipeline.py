...
def test_ingestion_pipeline_init(mock_settings):
    ...
    with patch('qdrant_loader.core.ingestion_pipeline.get_settings', return_value=mock_settings) as mock_get_settings:
...
async def test_process_documents_no_sources(mock_settings, mock_config):
    ...
    with patch('qdrant_loader.core.ingestion_pipeline.get_settings', return_value=mock_settings):
        pipeline = IngestionPipeline()
...
@pytest.mark.asyncio
@patch('qdrant_loader.core.ingestion_pipeline.EmbeddingService')
@patch('qdrant_loader.core.ingestion_pipeline.QdrantManager')
@patch('qdrant_loader.core.ingestion_pipeline.get_settings')
@patch('qdrant_loader.core.ingestion_pipeline.ChunkingStrategy')
async def test_process_documents_with_jira(mock_chunking_cls, mock_get_settings, mock_qdrant_cls, mock_embedding_cls):
    ...
    with patch('qdrant_loader.core.ingestion_pipeline.JiraConnector', return_value=mock_connector):
...
async def test_process_documents_with_jira():
    ...
    # Mock get_settings
    with patch('qdrant_loader.core.ingestion_pipeline.get_settings') as mock_get_settings:
        mock_get_settings.return_value = settings
