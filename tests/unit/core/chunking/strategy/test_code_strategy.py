...
from qdrant_loader.core.chunking.strategy.code_strategy import CodeChunkingStrategy, CodeElementType
...
class TestCodeChunkingStrategy:
    ...
    @patch("qdrant_loader.core.text_processing.semantic_analyzer.SemanticAnalyzer")
    def test_detect_language_java(self, mock_semantic_analyzer, mock_settings, sample_java_code):
    ...
    @patch("qdrant_loader.core.text_processing.semantic_analyzer.SemanticAnalyzer")
    def test_parse_python_ast(self, mock_semantic_analyzer, mock_settings, sample_python_code):
        ...
        # Check for imports
        import_elements = [e for e in elements if e.element_type == CodeElementType.IMPORT]
        assert len(import_elements) >= 2  # os, sys, typing imports

        # Check for class
        class_elements = [e for e in elements if e.element_type == CodeElementType.CLASS]
    ...
    @patch("qdrant_loader.core.text_processing.semantic_analyzer.SemanticAnalyzer")
    def test_tree_sitter_parsing(self, mock_semantic_analyzer, mock_settings, sample_python_code):
    ...
    def test_chunk_python_document(
        self, mock_text_processor, mock_semantic_analyzer, mock_settings, sample_python_code
    ):
        """Test chunking a Python document."""
        # Mock the text processor
        mock_text_processor_instance = Mock()
        mock_text_processor_instance.process_text.return_value = {"entities": [], "pos_tags": []}
        ...
        for chunk in chunks:
            ...
            assert "element_type" in chunk.metadata or "chunking_method" in chunk.metadata
    ...
    def test_chunk_java_document(
        self, mock_text_processor, mock_semantic_analyzer, mock_settings, sample_java_code
    ):
    ...
    @patch("qdrant_loader.core.text_processing.semantic_analyzer.SemanticAnalyzer")
    @patch("qdrant_loader.core.text_processing.text_processor.TextProcessor")
    def test_fallback_chunking(self, mock_text_processor, mock_semantic_analyzer, mock_settings):
