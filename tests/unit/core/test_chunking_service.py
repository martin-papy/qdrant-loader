...
def test_invalid_chunk_size(test_global_config, test_settings):
    ...
    with patch(
        "qdrant_loader.core.chunking_service.ChunkingService.__new__"
...
def test_chunk_document_error_handling(test_global_config, test_settings):
    ...
    with patch(
        ...
        # Test error handling with a valid Document object that causes an error
        invalid_doc = Document(
            content="",  # This will cause an error in the chunking strategy
            source="test_source",
            source_type="test_type",
            metadata={},
            created_at=datetime.now(),
        ...
        with pytest.raises(ValueError, match="Invalid document"):
            service.chunk_document(invalid_doc)

        # Verify the chunking operation
        mock_service.chunk_document.assert_called_once_with(invalid_doc)
